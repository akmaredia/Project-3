...
import collections
import tweepy
import aziz_twitter_info
import json
import sqlite3

## Your name: Aziz Maredia
## The names of anyone you worked with on this project: Myself
...
def get_user_tweets(user_handle):
	...
	unique_id = "twitter_{}".format(user_handle)

	if unique_id in CACHE_DICTION:
		twitter_results = CACHE_DICTION[unique_id]
	
	else:
		twitter_results = api.user_timeline(user_handle) # user_timeline only gives data on 20 tweets, so no need to index return statement
...
connection = sqlite3.connect('project3_tweets.db')
...
for element in umich_tweets:
    ...
    cursor.execute(statement, (element['id_str'], element["text"], element["created_at"], element["retweet_count"], element['user']['id_str']))
...
for tweet in umich_tweets:
	...
	for data in tweet["entities"]["user_mentions"]:
		information = api.get_user(id = data["id_str"])
		
		user_id.append(information["id_str"])
		screen_name.append(information["screen_name"])
		num_favs.append(information["favourites_count"])
		description.append(information["description"])
...
cursor.execute('SELECT screen_name FROM Users')
screen_names = [sn[0] for sn in cursor.fetchall()]
...
cursor.execute('SELECT * FROM Tweets WHERE retweets >= 25')
more_than_25_rts = cursor.fetchall()
...
cursor.execute('SELECT description FROM Users WHERE num_favs >= 25')
descriptions_fav_users = [u[0] for u in cursor.fetchall()]
...
most_common_char = collections.Counter(string.replace(" ", "")).most_common(1)[0][0]
